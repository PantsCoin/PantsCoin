Pantscoin Core version *0.15.1* is now available from:

  <https://download.pantscoin.org/pantscoin-0.15.1/>
...
A number of changes to the way Pantscoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 pants per kB (0.00001 PANTS). This is to prepare for dropping the minimum transaction fee to 0.1 pants per kB (0.0001 PANTS) in the next release.
