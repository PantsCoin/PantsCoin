...
const char * const BITCOIN_CONF_FILENAME = "pantscoin.conf";
const char * const BITCOIN_PID_FILENAME = "pantscoin.pid";
...
#else
    const char* pszModule = "pantscoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Pantscoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Pantscoin";
#else
    // Unix
    return pathRet / ".pantscoin";
#endif
#endif
}
